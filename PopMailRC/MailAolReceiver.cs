...
namespace MailExposure.PopMailRC
{
    class MailAolReceiver:MailStr
    {
                ...
                while ((startIndex = message.IndexOf("<tr", startIndex)) != -1)
                {
                    ...
                    if ((mailId != "-1") && (mailId != ""))
                    {
                        ...
                        if (Convert.ToInt32(GlobalValue.PopMainForm.ExecuteSQL(strSql)) == 0)
                        {
                            ...
                            try
                            {
                                DateTime emailDateTime = GlobalValue.PopMainForm.EmailDateTime;
                            ...
                            if (flag2)
                            {
                                this.url = base.Host + "/32319-211/aol-1/en-us/mail/ViewSource.aspx?user=" + this.user + "&folder=" + folder + "&uid=" + mailId;
                                base.streamControl = true;
                                try
                                {
        ...
        private bool getNextPage(string urls, string boxName, string boxId, int mailCount, string folder)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                if (urls.Equals(strArray[j]))
                {
                    ...
                    else if (numArray[j + 1] <= Convert.ToInt32(GlobalValue.PopMainForm.PageNumber))
                    {
                        base.MyStringBuilder.Remove(0, base.MyStringBuilder.Length);
                        base.streamControl = true;
                        base.MyStringBuilder = this.Request(strArray[j + 1]);
        ...
        public override void login()
        {
            ...
            string str3 = "";
            int startIndex = 0;
            startIndex = message.IndexOf("name=\"siteState\"");
            if (startIndex > 0)
            {
                str2 = HttpUtility.UrlEncode(base.putstr(message, "value=\"", "\"", startIndex));
            }
            startIndex = message.IndexOf("name=\"usrd\"");
            if (startIndex > 0)
            {
                str3 = base.putstr(message, "value=\"", "\"", startIndex);
            }
            startIndex = message.IndexOf("lsoDP");
            if (startIndex > 0)
            {
                base.putstr(base.cookie, "SNS_DP", ";", startIndex);
            }
            if (((str2 != "") && (str3 != "")) && ((str2 != "-1") && (str3 != "-1")))
            {
                this.url = "https://my.screenname.aol.com/_cqr/login/login.psp";
                string indata = "sitedomain=sns.webmail.aol.com&siteId=&lang=en&locale=us&authLev=0&siteState=" + str2 + "&isSiteStateEncoded=true&mcState=initialized&uitype=std&use_aam=0&_sns_fg_color_=&_sns_err_color_=&_sns_link_color_=&_sns_width_=&_sns_height_=&_sns_bg_color_=&offerId=newmail-en-us-v2&seamless=novl&regPromoCode=&usrd=" + str3 + "&doSSL=&redirType=&xchk=false&tab=&lsoDP=id%3DC3577930-1947-15A7-78B0-9C1A19AA2176&loginId=" + base.m_username + "&password=" + base.m_passwd;
                base.MyStringBuilder.Remove(0, base.MyStringBuilder.Length);
                base.streamControl = true;
                base.MyStringBuilder = this.PostData(this.url, indata);
                message = base.MyStringBuilder.ToString();
                if (message.IndexOf("Invalid Username or Password. Please try again.") != -1)
                {
                    base.LoginFail();
                    base.passwdErr();
                    base.ShowMessage("用户名或密码错误！");
                }
                else if (message.IndexOf("checkErrorAndSubmitForm(") != -1)
                {
                    string[] strArray = base.putstr(message, "checkErrorAndSubmitForm('", ")", 0).Split(new char[] { ',' });
                    ...
                    if ((this.url != "") && (this.url != "-1"))
                    {
                        ...
                        base.MyStringBuilder.Remove(0, base.MyStringBuilder.Length);
                        base.MyStringBuilder = this.Request(this.url);
                        string str6 = base.cookie.ToString();
                        this.user = base.putstr(str6, "uid:", "&", 0);
                        message = base.MyStringBuilder.ToString();
                        int index = 0;
                        base.Host = "http://mail.aol.com";
                        index = message.IndexOf("var gSuccessURL");
                        if (index > 0)
                        {
                            this.urls = base.Host + base.putstr(message, "\"", "\"", index);
                            base.streamControl = true;
                            base.MyStringBuilder.Remove(0, base.MyStringBuilder.Length);
                            base.MyStringBuilder = this.Request(this.urls);
                            if (base.MyStringBuilder.ToString().IndexOf("Lite/Today.aspx") > 0)
                            {
