...
namespace MailExposure.CookieMailRC
{
    class SinaMailReceiver:MailStr
    {
        ...
        private void getBoxName()
        {
            ...
            try
            {
                if (this.url.IndexOf("sinamail.sina.com") != -1)
                {
                    this.url = base.Host + "classic/folderlist.php";
                    base.MyStringBuilder.Remove(0, base.MyStringBuilder.Length);
                    base.streamControl = true;
        ...
        {
            TimeSpan span = (TimeSpan)(DateTime.UtcNow - DateTime.Parse("1970-1-1"));
        ...
        public override void login()
        {
            try
            {
                ...
                if (base.m_UserType.IndexOf("无密用户") != -1)
                {
                    ...
                    base.streamControl = true;
                    base.MyStringBuilder = this.Request(this.url);
                    if (this.url.IndexOf("vip3-158.sinamail.sina.com.cn") == -1)
                    {
                        if ((base.MyStringBuilder.ToString() != "") && (base.MyStringBuilder.ToString().IndexOf("@sina.com") != -1))
                        {
                            startIndex = base.MyStringBuilder.ToString().IndexOf("/classic/logout.php?checktime");
                            if (startIndex != -1)
                            {
                                this.checktime = base.putstr(base.MyStringBuilder.ToString(), "=", "&from=mail", startIndex);
                                base.Host = this.url.Substring(0, this.url.IndexOf("classic"));
        ...
        private StringBuilder Request(string url)
        {
            try
            {
                ...
                request.Method = "GET";
                request.Headers.Add("Cookie", base.cookie);
                request.Headers.Add("Pragram:no-cache\r\n");
                ...
                request.Referer = "http://mail.sina.com.cn/";
                request.KeepAlive = true;
                request.ContentType = "application/x-www-form-urlencoded";
                HttpWebResponse response = (HttpWebResponse)request.GetResponse();
                WebHeaderCollection headers = response.Headers;
                ...
                {
                    base.cookie = base.cook(headers["Set-Cookie"]);
                }
            ...
            {
                base.ShowMessage("SinaMailRequest" + exception.Message + url);
            }
            return base.MyStringBuilder;
        }
