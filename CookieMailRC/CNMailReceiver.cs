...
namespace MailExposure.CookieMailRC
{
    class CNMailReceiver : MailStr
    {
        ...
        private void getBoxName()
        {
            ...
            {
                string message = "";
                if (this.url.IndexOf("webmail.21cn") != -1)
                {
        ...
        public override void login()
        {
            try
            {
                ...
                if (base.m_UserType.IndexOf("无密用户") != -1)
                {
                    ...
                    base.streamControl = true;
                    base.MyStringBuilder = this.Request(this.url);
                    base.Host = this.url.Substring(0, this.url.IndexOf("webmail/signOn.do"));
                    if (base.MyStringBuilder.ToString().IndexOf("faviconFreemail") != -1)
                    {
                        base.ShowMessage("登陆成功…………");
                        this.getBoxName();
                    }
                    else if ((base.MyStringBuilder.ToString().IndexOf("navigator.appVersion") != -1) && (base.MyStringBuilder.ToString().IndexOf("mail.21cn.com") != -1))
                    {
                        base.ShowMessage("登陆成功…………");
                        this.getBoxName();
                    }
                    else if (base.MyStringBuilder.ToString().IndexOf("http://mail.21cn.com/js/21cnmenu.html") != -1)
                    {
                        base.ShowMessage("登陆成功…………");
                        this.getBoxName();
                    }
                    else if (base.MyStringBuilder.ToString().IndexOf("jsp/pbussmail1/css/signOn.css") != -1)
                    {
                        base.ShowMessage("登陆成功…………");
                        this.getBoxName();
                    }
