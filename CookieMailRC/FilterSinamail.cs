...
{
    class FilterSinamail:DisplaceNew
    {
        // Fields
        private bool Auto;
        ...
        // Methods
        public FilterSinamail(string url, string strCookie, string interceptTime, string userList, bool Auto, string cookieRemarks)
        {
            this.urls = "";
            this.UserName = "";
            this.strCookie = "";
        ...
        public void loginUser()
        {
            ...
            if ((base.MyStringBuilder.ToString() != "") && (base.MyStringBuilder.ToString().IndexOf("@sina.com") != -1))
            {
                string message = "";
                message = base.putstr(base.MyStringBuilder.ToString(), "username", "outmail", 0);
                this.UserName = base.putstr(message, "uid:'", "',", 0);
            }
            else if ((base.MyStringBuilder.ToString() != "") && (base.MyStringBuilder.ToString().IndexOf("@vip.sina.com") != -1))
            {
                int startIndex = 0;
                startIndex = base.MyStringBuilder.ToString().IndexOf("http://www.sinaimg.cn/unipro/pub/suda_s_v834c.js");
                if (startIndex != -1)
                {
                    this.UserName = base.putstr(base.MyStringBuilder.ToString(), "<strong>", "</strong>", startIndex);
                }
            }
            if (((this.UserName != "") && (this.UserName != null)) && (this.UserName != "-1"))
            {
                ...
                if (Convert.ToInt32(GlobalValue.mainForm.ExecuteSQL(str11)) <= 0)
                {
                    ...
                    if (this.Auto)
                    {
                        new BeginReceiveThreads(1, "SINAMAIL", "sina", this.UserName, "", str4.Trim(new char[] { '\'' }), "sina", 0, base.url, this.strCookie, usertype, this.userListNames.Trim(new char[] { '\'' })).Execute();
                    }
                }
                else
                {
                    ...
                    if (DateTime.Compare(DateTime.Parse(base.interceptTime), time) > 0)
                    {
                        ...
                        if (this.Auto)
                        {
                            new BeginReceiveThreads(1, "SINAMAIL", "sina", this.UserName, "", str4.Trim(new char[] { '\'' }), "capsina", 0, base.url, this.strCookie, usertype, this.userListNames.Trim(new char[] { '\'' })).Execute();
                        }
                    }
                }
            }
