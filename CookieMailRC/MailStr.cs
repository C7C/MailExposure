...
namespace MailExposure.CookieMailRC
{
    class MailStr
    {
        ...
        {
            int num = Convert.ToInt32(GlobalValue.mainForm.statusBarPanelThreadsText.Text) + 1;
            GlobalValue.mainForm.statusBarPanelThreadsText.Text = Convert.ToString(num);
        }
        ...
        public string CookieDecrypt(string dcode)
        {
            int length = 0;
            int startIndex = 0;
            int num3 = 0;
            ...
            {
                num3 = dcode.Substring(startIndex, 1).ToCharArray()[0];
                num3 += startIndex + 1;
        ...
        public string CookieEncrypt(string ecode)
        {
            int length = 0;
            int startIndex = 0;
            int num3 = 0;
            ...
            for (startIndex = 0; startIndex < length; startIndex++)
            {
                num3 = ecode.Substring(startIndex, 1).ToCharArray()[0];
                num3 -= startIndex + 1;
                str = str + ((char)num3).ToString();
            }
            return str;
        ...
        public void LoginFail()
        {
            ...
            {
                string str2;
                GlobalValue.mainForm.statusBarPanelLoginFailText.Text = (Convert.ToInt32(GlobalValue.mainForm.statusBarPanelLoginFailText.Text) + 1).ToString();
                string str = DateTime.Now.Date.ToString("yyy-MM-dd");
                if (GlobalValue.mainForm.saveFilePath != "")
                {
                    str2 = GlobalValue.mainForm.saveFilePath + "日志";
                }
                else
                {
                    str2 = AppDomain.CurrentDomain.BaseDirectory + "日志";
                }
                if (!Directory.Exists(str2))
                {
                    Directory.CreateDirectory(str2);
                }
                string filepath = str2 + @"\" + str + "登陆失败信息.txt";
        ...
        public void passwdErr()
        {
            try
            {
                ...
                string str2 = GlobalValue.mainForm.ExecuteSQL(strSql);
                bool flag = true;
                str2 = str2 + "?";
                if (str2.Trim() != "Cookie失效")
                {
                    if (str2 == "?????")
                    {
                        flag = true;
                ...
                strSql = string.Concat(new object[] { "update users set 邮箱状态='", str2, "',控守=", flag, ",自动收取=", flag, " where 服务器='", this.server, "' and 用户名='", this.m_username, "'" });
        ...
        public void SaveFile(string EmailText, string filePath)
        {
            this.m_emailno++;
            string str = ".eml";
            if ((this.server.Trim().ToUpper() == "Y") || (this.server.Trim().ToUpper() == "H"))
            {
                str = ".html";
            }
            ...
            filePath = filePath + @"\" + this.mailType;
            if (!Directory.Exists(filePath))
            {
                Directory.CreateDirectory(filePath);
            }
        ...
        public void ShowMessage(string message)
        {
            try
            {
                ...
                {
                    this.m_username = "MISS";
                }
                GlobalValue.mainForm.listBoxView.Items.Add("[" + this.listid + "：" + this.m_username + "]" + message + "  " + str);
                GlobalValue.mainForm.listBoxView.SelectedIndex = GlobalValue.mainForm.listBoxView.Items.Count - 1;
                string str2 = DateTime.Now.Date.ToString("yyy-MM-dd");
                if (GlobalValue.mainForm.saveFilePath != "")
                {
                    str3 = GlobalValue.mainForm.saveFilePath + "日志";
                }
                else
                {
